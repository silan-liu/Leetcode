...
class Solution {
    ...
    func reorderLogFiles(_ logs: [String]) -> [String] {
        ...
        for log in logs {
            var substrings = log.split(separator: " ")
            if substrings.count > 1 {
                let str = String(substrings[1])
        ...
        for (_, value) in sortedMap {
            // 内部也需要排序
            result.append(contentsOf: value.sorted())
        }
        
        result.append(contentsOf: digitArray)
        
        return result
    }
    
    // 26.98%
    func reorderLogFiles2(_ logs: [String]) -> [String] {
        var digitArray = [String]()
        ...
        // 对 letterArray 进行排序
        let sortedLetterArray = letterArray.sorted { (log1, log2) -> Bool in
            let (id1, str1) = getString(log1)
            let (id2, str2) = getString(log2)

            // 如果相等，则比较 identitifier
            if str1 == str2 {
                return id1 < id2
            }
        ...
        print(sortedLetterArray)
        
        return sortedLetterArray + digitArray
    }
    
    func getString(_ str: String) -> (String, String) {
        let substringList = str.split(separator: " ")
        let identifier = String(substringList[0])

        var letters = ""
        var i = 1
        while i < substringList.count {
            letters += substringList[i]
